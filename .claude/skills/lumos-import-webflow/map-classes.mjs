...
for (const file of walk(srcDir, [".css", ".astro"])) {
  ...
  componentNames.set(
    name.toLowerCase(),
    file.replace(`${srcDir}/components/`, ""),
  );
...
for (const [name, count] of [...used].sort((a, b) => b[1] - a[1])) {
  ...
  if (hit) known.push([name, count, hit]);
    ...
    components.push([name, count, owner.get(name).file]);
...
for (const [head, f] of families) {
  if (f.classes.length < 2) {
    families.delete(head);
    continue;
  }
...
if (asSed) {
  console.log("#!/bin/sh");
  console.log(
    "# Renames Lumos for Webflow utilities to this framework's names.",
  );
...
if (components.length > 12)
  console.log(`  … and ${components.length - 12} more`);
...
if (families.size) {
  ...
  console.log(
    "  Swap the markup for the component, then style the component to match.",
  );
}
...
console.log(`\n${rule}`);
console.log(
  "Rename what renames, swap the component where one owns the class, and",
);
console.log(
  "keep the rest — they are the site's own and their CSS already came across.",
);
