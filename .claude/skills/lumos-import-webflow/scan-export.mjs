...
function topLevelBlocks(html, depthLimit = 2) {
  ...
  const dominant = blocks.find(
    (b) => b.html.length > total * 0.8 && b.html.length > 4000,
  );
  if (dominant && depthLimit > 0) {
    const inner = dominant.html
      .replace(/^<[^>]+>/, "")
      .replace(/<\/[a-z]+>\s*$/i, "");
    return [
      {
        name: `${dominant.name} (wrapper)`,
        html: dominant.html,
        wrapper: true,
      },
...
for (const file of files) {
  ...
  for (const form of html.matchAll(/<form\b([^>]*)>([\s\S]*?)<\/form>/g)) {
    ...
    const fields = [];
    for (const input of form[2].matchAll(
  ...
  const variantClasses = (html.match(/\bw-variant-[0-9a-f-]{8,}/g) ?? [])
    .length;
  if (variantClasses) {
  ...
  for (const m of html.matchAll(/(data-wf-[a-z-]+)="([^"]*)"/g)) {
    const key = m[1];
    if (/^data-wf-(site|page|page-id|element-id|user-form-type)$/.test(key))
      continue;
    if (key.startsWith("data-wf--")) continue;
    report.componentMarkers[key] ??= {};
    report.componentMarkers[key][m[2]] =
      (report.componentMarkers[key][m[2]] ?? 0) + 1;
  }
  ...
  for (const m of html.matchAll(
    /\b(data-(?:xsmall|small|medium|large)-columns|data-trigger|data-button|data-hide-from|data-code-move|data-number)="?/g,
  )) {
  ...
  for (const m of html.matchAll(
    /@media[^{]*?\(\s*(?:width\s*<|max-width:)\s*([\d.]+)(em|rem|px)\s*\)/g,
  )) {
...
for (const p of report.pages) {
  console.log(
    `  ${p.route.padEnd(24)} ${p.page.padEnd(24)} ${p.pageId ?? "(no id)"}`,
  );
}
...
for (const [name, { pages, target }] of Object.entries(report.widgets)) {
  console.log(
    `  ${name.padEnd(14)} ${String(pages.length).padStart(3)} page(s)  →  ${target}`,
  );
}
...
for (const d of report.dynamicLists) {
  console.log(
    `  ${d.page.padEnd(28)} ${d.lists} list(s), ${d.renderedItems} rendered item(s)${d.hasEmptyState ? ", has empty state" : ""}`,
  );
}
...
for (const f of report.forms) {
  console.log(
    `  ${f.page.padEnd(28)} "${f.name}" ${f.method.toUpperCase()} — ${f.fields.length} field(s): ${f.fields.map((x) => x.name + (x.required ? "*" : "")).join(", ")}`,
  );
}
...
for (const i of report.interactions) {
  console.log(
    `  ${i.page.padEnd(28)} ${i.animatedElements} animated element(s)`,
  );
}
...
if (breaks.length) {
  ...
  console.log(
    "  Webflow writes these as max-width, shrinking down. Inverted to the",
  );
  console.log("  min-width form this framework uses, the design breaks at:");
  for (const [rem, n] of breaks) {
    console.log(
      `    @media (width >= ${rem}rem)   ${String(rem * 16).padStart(5)}px   (${n} rule(s))`,
    );
  }
  console.log(
    "  The framework ships 30rem / 48rem / 64rem. To reflow where the",
  );
  console.log(
    "  original did, change those literals — they are not tokens, so they",
  );
...
if (chrome.length) {
  ...
  console.log(
    "  ONCE into BaseLayout, Global/Nav or Global/Footer. A block that",
  );
  console.log(
    "  differs per page is usually a wrapper — keep the wrapper in the",
  );
...
const comps = Object.entries(report.components).sort(
  (a, b) => b[1].instances - a[1].instances,
);
if (!comps.length) {
...
const variantTotal = Object.values(report.variantInstances).reduce(
  (a, b) => a + b,
  0,
);
if (variantTotal) {
  console.log(
    `  plus ${variantTotal} w-variant-* class(es) across ${Object.keys(report.variantInstances).length} page(s) —`,
  );
  console.log(
    "  a variant id with no name; resolve it with list_components if it matters",
  );
}
...
if (lumos.length) {
  ...
  console.log(
    `  ${lumos.length} u-* classes, ${lumos.reduce((a, [, n]) => a + n, 0)} uses. Most common:`,
  );
  for (const [cls, n] of lumos.slice(0, 8))
...
if (libs.length) {
  ...
  for (const [lib, pages] of libs.sort((a, b) => b[1].size - a[1].size)) {
    console.log(
      `  ${lib.padEnd(12)} on ${String(pages.size).padStart(3)} page(s)`,
    );
  }
}
...
if (report.embeds.length) {
  ...
  for (const e of report.embeds)
    console.log(`  ${e.page.padEnd(28)} ${e.embeds} embed(s)`);
}
